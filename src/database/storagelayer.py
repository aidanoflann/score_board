...
def connect():
    ...
    db = MySQLdb.connect(host=settings.MYSQL_DB_URL, user=settings.MYSQL_USERNAME, passwd=settings.MYSQL_PASSWORD)
    cursor = db.cursor()
    sql = 'CREATE DATABASE IF NOT EXISTS {}'.format(settings.MYSQL_SCOREBOARD_DB_NAME)
    cursor.execute(sql)
    db = MySQLdb.connect(host=settings.MYSQL_DB_URL, user="root", passwd="my-secret-pw", db="test")
    if db is None:
