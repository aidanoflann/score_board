...
def connect():
    ...
    db = MySQLdb.connect(host="score_board_and_db", user=settings.MYSQL_USERNAME, passwd=settings.MYSQL_PASSWORD)
    cursor = db.cursor()
    sql = 'CREATE DATABASE IF NOT EXISTS {}'.format(settings.MYSQL_SCOREBOARD_DB_NAME)
    cursor.execute(sql)
    db = MySQLdb.connect(host="score_board_and_db", user="root", passwd="my-secret-pw", db="test")
    if db is None:
